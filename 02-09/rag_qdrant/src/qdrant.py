...
def recreate_collection_for_rag(
    client: QdrantClient, settings: Settings, vector_size: int
):
    """
    ...
    client.recreate_collection(
        ...
        quantization_config=ScalarQuantization(
            ...
            )  # on-disk quantization dei vettori
        ),
    )
...
def build_points(
    chunks: list[Document], embeds: list[list[float]]
) -> list[PointStruct]:
...
def qdrant_text_prefilter_ids(
    client: QdrantClient, settings: Settings, query: str, max_hits: int
) -> list[int]:
    """
...
def hybrid_search(
    ...
    text_ids = set(
        qdrant_text_prefilter_ids(client, settings, query, settings.top_n_text)
    )
